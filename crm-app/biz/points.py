...
from common.biz.points import points_rounding
...
async def produce_points_event(app, crm_id, member_no, points, event_data, event_at):
    """处理积分增加的事件"""
    # event_data 里面的积分过期时间 计算到当天的24点
    try:
        expire_time = event_data.get("expire_time")
        if expire_time:
            fix_expire_time = expire_time.strftime("%Y-%m-%d 23:59:59")
            event_data['expire_time'] = fix_expire_time
    except Exception as ex:
        logger.exception(ex)
        logger.warning(f"fix_expire_time_error")
    # 事务处理
    try:
        ...
        logger.exception(ex)
        await t.rollback()
        return dict(code=RC.DATA_EXCEPTION, msg="增加积分处理失败")
...
async def cal_consume_events_ids(app, crm_id, member_no, event_at, target_points, event_type="consume"):
    ...
    if event_type == "transfer":
        ...
        sql_str = _model.select().where(*where). \
            order_by(_model.expire_time.asc(), _model.auto_id)
    used_points = 0
    for page_id in range(1, page_num + 1):
        ...
        for _id in ids_li:
            ...
            if all_point >= target_points:
                ...
                consume_ids_li.append(_id)
                used_points += one_points
    logger.info(f"{member_no} consume points={target_points} used_points={used_points} need process ids={consume_ids_li} ")
...
async def reverse_produce_event(app, crm_id, member_no, origin_event_no, event_data, part_reverse=False,
    ...
    try:
        async with app.mgr.atomic() as t:
            ...
            points = one.get("points", 0)
            # 可用积分不够冲正
            if points < change_points:
                # 计算可冲正的积分事件 可用积分中选择快过期的事件
                tgt_points = change_points - points
                consume_ids_li = await cal_unexpired_events_ids(app, crm_id, member_no, event_at,
                                                                target_points=tgt_points)
                logger.info(f"{event_no} 原事件积分不够冲正 将要冲正的事件{consume_ids_li}")
                # 删除原冲正事件 更改冲正的积分为 当前事件积分
                event_data['points'] = points
                await add_record_from_dict(app.mgr, PointsHistory, event_data)
                # 删除原可用积分表
                await app.mgr.execute(PointsAvailable.delete().where(
                    PointsAvailable.event_no == origin_event_no
                ))
                # 修改可用积分表, 记录每次冲正事件
                _model = PointsAvailable
                # 冲正事件
                other_reverse_events = []
                all_freeze_points, all_future_expired, all_active_points = 0, 0, 0
                for _index, point_info in enumerate(consume_ids_li):
                    # 删除或修改
                    auto_id = point_info['auto_id']
                    used = point_info['used']
                    remain = point_info['remain']
                    this_event = point_info.get("event_no")
                    if remain:  # 修改
                        logger.info(f"update PointsAvailable points={remain} where auto_id={auto_id}")
                        got = await app.mgr.execute(_model.update(points=remain).where(
                            _model.points >= used, _model.auto_id == auto_id
                    ...
                    if not got:
                        await t.rollback()
                        logger.info(f"删除或更新可用积分报错")
                        return dict(code=RC.HANDLER_ERROR, msg="冲正删除或更新其他事件积分报错 稍后重试")
                    # 计算积分概览
                    expire_time = point_info['expire_time']
                    unfreeze_time = point_info['unfreeze_time']
                    freeze_points, _, future_expired, active_points = \
                        process_points_change(used, event_at, unfreeze_time, expire_time)
                    all_freeze_points += freeze_points
                    all_future_expired += future_expired
                    all_active_points += active_points
                    # 计算可用积分时间的冻结过期时间
                    tmp_data = deepcopy(event_data)
                    tmp_data.update(dict(event_no=f"{event_no}-{_index+1}", points=used, redo_use_event=this_event))
                    tmp_obj = peewee_normalize_dict(PointsHistory, tmp_data)
                    other_reverse_events.append(tmp_obj)

                # 批量写入points_his
                if other_reverse_events:
                    await app.mgr.execute(PointsHistory.insert_many(other_reverse_events).on_conflict_ignore())
                    logger.info(f"insert many other_reverse_events={other_reverse_events}")
                # 修改积分概览表
                await app.mgr.execute(PointsSummary.update(
                    freeze_points=PointsSummary.freeze_points - all_freeze_points,
                    future_expired=PointsSummary.future_expired - all_future_expired,
                    active_points=PointsSummary.active_points - all_active_points,
                ).where(PointsSummary.member_no == member_no, PointsSummary.crm_id == crm_id))
                logger.info(
                    f"summay changes {member_no} freeze_points-{all_freeze_points} future_expired-{all_future_expired} active_points-{all_active_points}")
            else:
                if part_reverse:
...
async def calculate_add_points_by_rules(app, crm_id, member_no, event_at, rule_items, params_dict, action_scene):
    ...
    if not rule_items:
        logger.info(f"action_scene:{action_scene} no rule_items")
    ...
    if action_scene == "order_pay":
        ...
        order_info = params_dict.get("order_info") or {}
        total_amount = order_info.get("pay_amount")
        this_rule = {}
        for i in sort_rules:
        ...
        if not this_rule:
            # 没有匹配到阶梯规则 使用最小的那个
            this_rule = sort_rules[-1]
        # this rule 添加积分
        if this_rule:
            ratio_points = this_rule.get('ratio_points')
            # 按金额的倍率来算
            if ratio_points:
                total_amount = total_amount * ratio_points
            else:
                per_money = this_rule.get('per_money')
                change_points = this_rule.get('change_points')
                total_amount = Decimal(str(total_amount))
                per_money = Decimal(str(per_money))
...
async def cal_unexpired_events_ids(app, crm_id, member_no, event_at, target_points, event_type="consume"):
    """计算用户的未过期积分(可用待解冻) 转赠中的不能冲
    return 积分可用表数据+used+remain"""
    _model = PointsAvailable
    where = [
        (_model.expire_time >= event_at) | (_model.expire_time == None),
    ...
    for page_id in (1, page_num + 1):
        ...
        for _id in ids_li:
            ...
            if all_point >= target_points:
                ...
                _id.update(dict(remain=0, used=one_points))
                consume_ids_li.append(_id)
    logger.info(f"{member_no} unexpired points={target_points} need process ids={consume_ids_li}")
    return consume_ids_li


async def handle_transfer_event_v2(app, crm_id, transfer_no, params, trans_points, member_from, member_to):
    """直接转赠积分的接口"""
    _model1 = PointsAvailable
    from_user = params.get("from_user")
    to_user = params.get("to_user")
    from_mobile = member_from.mobile
    to_mobile = member_to.mobile
    event_at = datetime.now()
    # 看积分是否够用
    act_points = await member_active_points(app.mgr, crm_id, from_user, event_at=event_at)
    if act_points < trans_points:  # 活跃积分< 要转赠积分
        return dict(code=RC.HANDLER_ERROR, msg="积分不足,转赠失败")
    event_ids_li = await cal_consume_events_ids(app, crm_id, from_user, event_at, trans_points)
    try:
        async with app.mgr.atomic() as t:
            # 将a的可用积分表 会员号 更新为b
            ...
            true_trans = 0
            for trans_detail in event_ids_li:
                auto_id = trans_detail.get("auto_id")
                remain = trans_detail.get("remain")
                used = trans_detail.get("used")
                if remain:
                    # 减少A用户，新增B用户
                    got = await app.mgr.execute(_model1.update(points=remain).where(
                        _model1.points >= used, _model1.auto_id == auto_id
                    ))
                    expire_time = trans_detail.get("expire_time")
                    unfreeze_time = trans_detail.get("unfreeze_time")
                    b_act_evnets = dict(
                        crm_id=crm_id, member_no=to_user, event_no=transfer_no, points=used,
                        expire_time=expire_time,unfreeze_time=unfreeze_time,event_at=event_at
                    )
                    await app.mgr.execute(_model1.insert(b_act_evnets))
                    logger.info(f"process_trans {from_user}->{to_user} update: auto_id={auto_id} points={remain}; insert={b_act_evnets}")
                    true_trans += used
                else:
                    replace_ids.append(auto_id)
                    true_trans += used
            # 批量修改 A 为 B
            if replace_ids:
                await app.mgr.execute(_model1.update(member_no=to_user).where(_model1.auto_id.in_(replace_ids)))
                logger.info(f"process_trans {from_user}->{to_user} replace: member_no={to_user},event_no={transfer_no} {replace_ids}")
            # 记录A->B的事件 history
            from_event_data = dict(
                crm_id=crm_id,
                member_no=from_user,
                ...
                points=trans_points,
                from_transfer_no=transfer_no,
                event_desc=f"积分赠送({to_mobile})"
                # expire_time=min_expire_time,  # 取最小过期时间
                # unfreeze_time=min_unfreeze_time,  # 取最小过期时间
            )
            await add_record_from_dict(app.mgr, PointsHistory, from_event_data, on_conflict=0)
            to_event_data = dict(
                crm_id=crm_id,
                member_no=to_user,
                event_no=f"A{transfer_no}",
                ...
                points=trans_points,
                from_transfer_no=transfer_no,  # 领取的哪个积分
                event_desc=f"获得{from_mobile}赠送的积分"
                # expire_time=min_expire_time,  # 取最小过期时间
                # unfreeze_time=min_unfreeze_time,  # 取最小过期时间
            )
            await add_record_from_dict(app.mgr, PointsHistory, to_event_data, on_conflict=0)
            # 写入到transfer表中
            event_data = dict(
                transfer_no=transfer_no,
                crm_id=crm_id,
                ...
                to_user=to_user,
                points=trans_points,
                points_detail=event_ids_li[:100],
                trans_type="by_points",
                event_at=event_at,
                done=1
            )
            await app.mgr.execute(PointsTransfer.insert(event_data))
            logger.info(f"transfer_no={transfer_no} trans_points={trans_points} true_trans={true_trans}")
            # todo 更新用户 A和用户B的积分概览
            await app.mgr.execute(PointsSummary.update(
                active_points=PointsSummary.active_points - trans_points,
            ).where(PointsSummary.member_no==from_user, PointsSummary.crm_id==crm_id))
            # B用户增加积分
            b_summary = dict(crm_id=crm_id, member_no=to_user, total_points=trans_points, active_points=trans_points)
            await app.mgr.execute(PointsSummary.insert(b_summary).on_conflict(
                update={
                    "total_points": PointsSummary.total_points + trans_points,
                    "active_points": PointsSummary.active_points + trans_points,
                }
            ))
            logger.info(f"summary_change from_user:{from_user} -{trans_points} to_user:{to_user} +{trans_points}")
            return dict(code=RC.OK, msg="转赠领取处理OK")
    except Exception as ex:
        await t.rollback()
        logger.exception(ex)
