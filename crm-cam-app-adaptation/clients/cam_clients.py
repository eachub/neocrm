...
class CamClient(object):
    ...
    async def _remote_request(self, concept, action, obj, instance_id, method=None, version=None, **kwargs):
        ...
        if '_' in action:
            ...
            if new_action:
                action = new_action
        url = f"{self._url_prefix}/{sub}/cam/{concept}/{instance_id}/{action}"
        if method is None:
            method = "GET" if action.split("_")[0] in \
                ("fetch", "search", "get", "query", "show", "list", "find") else "POST"
        ###
        logger.info(f"CAM-CLIENT request: {url}, {obj}")
        if method == "GET":
        ...
        ###
        logger.info(f"CAM-CLIENT response: {result}") 
        if result is None:
...
if __name__ == '__main__':
    ...
    client = CamClient(http, **PARAMS_FOR_CRM)
    # from_time = 1628958001
    loop.run_until_complete(
        client.member.rules__({"goods_id": 200001}, instance_id=158000)
    )
