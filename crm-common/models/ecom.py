...
class ProductMain(Model):
    ...
    class Meta:
        database = db_eros_crm
        table_name = "t_crm_product_main"

    id = BigAutoField()
    outer_goods_id = CharField(help_text="商品id")
    goods_name = CharField(help_text="商品名称")
    effect = CharField(null=True, help_text="功效名称")
    series_name = CharField(null=True, help_text="产品类别系列")
    outer_sku_code = CharField(unique=True, index=True, max_length=16, help_text="产品编码")
    out_spec = CharField(help_text="外包规格")
    net_weight = CharField(null=True, help_text="总净含量(g)")
    sales_unit = CharField(null=True, help_text="计价单位")
    reco_price = CharField(null=True, max_length=16, help_text="建议零售价")
    base_price = CharField(null=True, max_length=16, help_text="成本价")
    taste = CharField(null=True, max_length=16, help_text="口味")
    apply_people = CharField(null=True, max_length=16, help_text="适用人群")
    dosage_form = CharField(null=True, max_length=16, help_text="产品剂型")
    is_hf = BooleanField(default=False, help_text="是否保健食品：0否1是")
    extra = JSONField(null=True, help_text="其他")
    version = CharField(index=True, help_text="版本id")
    removed = BooleanField(default=False, help_text="是否删除：0否1是")
...
class CategoryNode(Model):
    class Meta:
        database = db_eros_crm
        table_name = "t_crm_category_node"
        auto_id_base = 90000
        indexes = (
            (('create_time', 'mall_id'), False),
            (('mall_id', 'parent_id'), False),
        )

    cat_id = AutoField(help_text="类目自增ID")
    mall_id = IntegerField(help_text="商城编号")
    name = CharField(max_length=64, help_text="类目名称")
    icon = CharField(max_length=64, null=True, help_text="类目图标")
    sort_num = IntegerField(default=50, help_text="排序号")
    remark = TextField(null=True, help_text="备注或其他")
    extra = JSONField(default={}, help_text="节点原始信息")
    parent_id = IntegerField(default=0,help_text="父节点ID：0表示根节点")
    layer = IntegerField(default=0, help_text="层级")
    path = CharField(128, null=True, help_text="路径")
    removed = BooleanField(default=0, help_text="是否已删除")
    create_time = DateTimeField(index=True, constraints=[auto_create])
    update_time = DateTimeField(index=True, constraints=[auto_create, auto_update])


class CategoryBind(Model):
    class Meta:
        database = db_eros_crm
        table_name = 't_crm_category_goods'
        indexes = (
            (('goods_id', 'cat_id'), True),
            (('cat_id', 'mall_id'), False),
        )
    auto_id = BigAutoField(help_text='流水ID')
    mall_id = IntegerField(help_text="商城编号")
    goods_id = BigIntegerField(help_text='商品编码')
    cat_id = IntegerField(help_text='类目ID')
    removed = BooleanField(default=0, help_text="是否已删除")
    create_time = DateTimeField(constraints=[auto_create])
    update_time = DateTimeField(constraints=[auto_create, auto_update])