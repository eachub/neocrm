...
class CrmClient(object):
    ...
    def __init__(self, http, url_prefix, url_mid="crm",**kwargs):
        self._http = http
    ...
    async def _remote_request(self, concept, action, obj, crm_id, method=None, version=None, preview_url=False, **kwargs):
        ...
        if '_' in action:
            ...
            if new_action:
                action = new_action
        url_mid = self._url_mid
        url = f"{self._url_prefix}/{sub}/{url_mid}/{concept}/{crm_id}/{action}"
...
class CamClient(object):
    ...
    async def _remote_request(self, concept, action, obj, instance_id, method=None, version=None, preview_url=False, **kwargs):
        """:param preview_url 预览url和请求方法"""
...
class CmsClient(object):
    ...
    async def _remote_request(self, concept, action, obj, instance_id, method=None, version=None, preview_url=False, **kwargs):
        """:param preview_url 预览url和请求方法"""
        assert concept and type(concept) is str, "cms-client：错误的参数concept"
        assert action and type(action) is str, "cms-client：错误的参数action"
        kwargs = dict(self._kwargs, **kwargs)
        sub = f"api/{version}" if version else "api"
        if '_' in action:
            new_action = action_maping.get(action)
            if new_action:
                action = new_action
        url = f"{self._url_prefix}/{sub}/cms/{concept}/{instance_id}/{action}"
        logger.info(f"method={method} url={url}")
        if preview_url:
            return True, dict(method=method, url=url, obj=obj, instance_id=instance_id)
        if method is None:
